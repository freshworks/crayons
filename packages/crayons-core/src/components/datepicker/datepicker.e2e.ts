...
describe('fw-datepicker', () => {
  ...
  it('should show alert icon when showErrorOnInvalidDate prop is set and invalid input is entered in the date input textbox', async () => {
    const page = await newE2EPage();
    await page.setContent(
    ...
    const alertElement = await shadow.find('.icon-calendar .invalid-alert');
    expect(alertElement).toBeTruthy();
  });

  it('should not show alert icon when showErrorOnInvalidDate prop is not set and invalid input is entered in the date input textbox', async () => {
    const page = await newE2EPage();
    await page.setContent('<fw-datepicker></fw-datepicker>');
    const input = await page.find('fw-datepicker >>> fw-input');
    await input.click();
  ...
  it('should emit the details in fwChange event when invalid input is entered in the date input textbox', async () => {
    const page = await newE2EPage();
    await page.setContent('<fw-datepicker></fw-datepicker>');
    const fwChange = await page.spyOnEvent('fwChange');
    const input = await page.find('fw-datepicker >>> fw-input');
