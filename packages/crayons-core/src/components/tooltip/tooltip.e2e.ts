...
describe('fw-tooltip', () => {
  ...
  it('should show tooltip with html if content is passed in a named slot', async () => {
    ...
    await page.setContent(
      '<fw-tooltip><fw-button> Show tooltip </fw-button><div slot="content">This tooltip has <b>HTML</b> content.</div></fw-tooltip>'
    );
    const tooltip: any = await page.find('fw-tooltip');
    tooltip.callMethod('show');
    await page.waitForChanges();
    const popoverContent: any = await page.find('fw-tooltip *[slot="content"]');
