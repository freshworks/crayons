(() => {
  ...
  window.$docsify.plugins.push((hook, vm) => {
    ...
    hook.mounted(function () {
      ...
      searchBox.innerHTML = `
        <fw-input
          ...
          clearInput
        >
          <fw-icon name="search"></fw-icon>
          <kbd slot="suffix" title="Press / to search">/</kbd>
        </fw-input>
      `;
      const searchBoxInput = searchBox.querySelector('fw-input');
        ...
        matches.map((match, index) => {
          ...
          a.innerHTML = `
            <div class="site-search__result-icon">
              <fw-icon name="${icon}" aria-hidden="true"></fw-icon>
            </div>
    ...
    input.addEventListener('fwInput', handleInput);
