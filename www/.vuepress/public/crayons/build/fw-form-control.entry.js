...
let FormControl = class {
      ...
      case 'DATE':
        {
          ...
          cmp = (h("fw-datepicker", Object.assign({}, componentProps, { readonly: true, ref: (el) => (this.crayonsControlRef = el) })));
        }
        break;
      case 'CHECKBOX':
        {
  ...
  render() {
    return (h("div", { class: 'form-control-container' }, this.renderControl(), this.hasSlot && (h("label", { htmlFor: this.name, class: {
      ...
      } }, this.label)), h("slot", null), this.hasSlot && !(this.touched && this.error) && (h("div", { class: 'hint', id: `hint-${this.name}` }, this.hint)), this.hasSlot && this.touched && this.error && (h("div", { class: 'error', id: `error-${this.name}` }, TranslationController.t(this.error, {
      field: this.label || this.name,
    })))));
  }
